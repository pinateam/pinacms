				...
				<tr class="first">
					<td>
						<div class="image-selector image-exists {if not $image} hidden{/if}" style="width:100%">
							<div class="image">
								<a href="{img img=$image.work_image_filename type="work_image"}" target="_blank">
								<img src="{img img=$image.work_image_filename type="work_image"}" alt="" id="image" width="155" />
								</a>
							</div>
						</div>
...
$(document).ready(function(){
	...
	var swfuSettings = {
		...
		post_params: {
			action: 'work.manage.image-upload',
